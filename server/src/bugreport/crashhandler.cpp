...
LPCTSTR __stdcall GetFaultReason( EXCEPTION_POINTERS* pExPtrs )
{
	...
	__try
	{
		...
		{
			iCurr += ( FormatMessage( FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_HMODULE, GetModuleHandle( _T( "NTDLL.DLL" ) ), pExPtrs->ExceptionRecord->ExceptionCode, 0, g_szBuff + iCurr, BUFF_SIZE, 0 ) * sizeof( TCHAR ) );
		}
...
#define CH_MACHINE IMAGE_FILE_MACHINE_I386
		...
		BOOL bSWRet = StackWalk( CH_MACHINE, GetCurrentProcess(), GetCurrentThread(), &g_stFrame, pExPtrs->ContextRecord, NULL, SymFunctionTableAccess, MySymGetModuleBase, NULL ) ;
		...
		*/
		...
		if ( GSTSO_MODULE == ( dwOpts & GSTSO_MODULE ) )
		{
			...
			dwTemp = MySymGetModuleBase( GetCurrentProcess(), g_stFrame.AddrPC.Offset ) ;
		...
		if ( GSTSO_SYMBOL == ( dwOpts & GSTSO_SYMBOL ) )
		{
			...
			if ( TRUE == SymGetSymFromAddr( GetCurrentProcess(), g_stFrame.AddrPC.Offset, &dwDisp, pSym ) )
			{
				iCurr += wsprintf( g_szBuff + iCurr, _T( ", " ) ) ;
		...
		if ( GSTSO_SRCLINE == ( dwOpts & GSTSO_SRCLINE ) )
		{
			...
			if ( TRUE == InternalSymGetLineFromAddr( GetCurrentProcess(), g_stFrame.AddrPC.Offset, &dwDisp, &g_stLine ) )
			{
				iCurr += wsprintf( g_szBuff + iCurr, _T( ", " ) ) ;
				...
				else
				{
					if ( dwDisp > 0 )
					{
						iCurr += wsprintf( g_szBuff + iCurr, _T( "%s:%d + %d byte(s)" ), g_stLine.FileName, g_stLine.LineNumber, dwDisp );
					}
					else
					{
						iCurr += wsprintf( g_szBuff + iCurr, _T( "%s:%d" ), g_stLine.FileName, g_stLine.LineNumber ) ;
					}
				}
			}
		}
...
BOOL CALLBACK EnumerateLoadedModulesProc( PSTR ModuleName, DWORD ModuleBase, ULONG ModuleSize, PVOID UserContext )
{
