					...
					else if (!(strcmp("EMT", (char*)script1)))  // Player says something when trigger is activated
					{
						...
						for (int i = 0; i < now; i++)
						{
							if (inrange1p(currchar[ts], currchar[i]) && perm[i])
							{
								...
								Xsend(i, talk, 14);
								Xsend(i, (void*)pc_ts->name.c_str(), 30);
...
//							unsigned int uiTimer = makenumber(1);
							...
							pos = ftell(scpfile);
							closescript();
							npcnum = Npcs->createScriptNpc(-1, pi, QString("%1").arg(uiNpcNum));
				...
				case 'E':
					...
					case 'N':
						...
						{// Usage: NADD <npc_number> <life_in_seconds>
							...
							pos = ftell(scpfile);
							closescript();
							npcnum = Npcs->createScriptNpc(-1, NULL, QString("%1").arg(uiNpcNum), triggerx, triggery, triggerz);
