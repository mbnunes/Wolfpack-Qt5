...
# baseid = [1st stage, 2nd stage....]
stages = {
	"wheat" : [ 0x1ebf, 0x1ebe, [0xc55, 0xc56], [0xc57, 0xc58], [0xc5a, 0xc5b]]
	}
...
def growCrop(crop):
	...
	# Set the new id
	if list(stages[base][stage]):
		new_id = random.choice(stages[base][stage])
		crop.say(" liste" )
	else:
		new_id = stages[base][stage]
	crop.id = new_id
	
	crop.update()
	# if it's ripe, remove it from growth check
	if crop.id in list(stages[base][-1]) or crop.id == stages[base][-1]:
		crop.say("UNREGISTER" )
		unregister(crop)
		crop.removescript("crops.growing")
		crop.addscript("cropt.harvest")
		crop.deltag( "stage" )
		console.log( 1, "unregister because it's ripe" )
...
#
def onAttach(obj):
	# only attach if not ripe
	if obj.id in list(stages[obj.baseid][-1]) or obj.id == stages[obj.baseid][-1]:
		return
	else:
		register(obj)
