...
def onLogin(player):
	...
	if not player.guild:
		...
		if len(online) > 1:
			player.socket.sysmessage(tr('There are %u other members of your guild logged on.') % len(online), 0x30)
		elif len(online) == 1:
			player.socket.sysmessage(tr('There is one other member of your guild logged on.'), 0x30)
		else:
			player.socket.sysmessage(tr('There is no other member of your guild logged on.'), 0x30)
...
def onDelete(player):
	guild = player.guild
	message = tr("'%s' has left the game because the character was deleted.") % player.name
