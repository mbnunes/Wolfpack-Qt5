...
def guildMemberDetails(player, guild, char):
  ...
  dialog.addText(160, 90, "Guildstone for %s%s" % (name, abbreviation), 380)
  dialog.addText(160, 113, "Member Details", 2100)
  ...
  dialog.addText(120, 220, "Member Since: %s" % strftime(FORMAT_DATETIME, localtime(info['joined'])), 2100)
  dialog.addText(120, 250, "Membership Status: %s" % status, 2100)

  if checkPermission(player, guild, PERMISSION_TITLES):
    dialog.addText(120, 280, "Guild Title:", 2100)
    dialog.addResizeGump(196, 276, 9300, 252, 29)
    dialog.addInputField(200, 279, 244, 23, 2100, 1, info['guildtitle'])
  else:
    dialog.addText(120, 280, "Guild Title: %s" % info['guildtitle'], 2100)

  if info['showsign']:
    dialog.addText(120, 310, 'Shows Guildsign: Yes', 2100)
  else:
    dialog.addText(120, 310, 'Shows Guildsign: No', 2100)
  ...
  dialog.addText(152, 288, "May act as an diplomat for this guild.", color[3])
  dialog.addText(152, 320, "May kick members out of this guild.", color[4])
  dialog.addText(152, 352, "May expel canidates from the guild.", color[5])
  dialog.addText(152, 384, "May grant guild titles to members of this guild.", color[6])
  dialog.addText(152, 416, "May grant permissions to other guild members.", color[7])

  dialog.startPage(3)
  dialog.addText(232, 160, "Miscellaneous Actions", 380)
  dialog.addResizeGump(112, 184, 9200, 380, 100)

  # Sending yourself a message is silly
  if char != player:
    dialog.addText(127, 192, "Send this member a private message:", 2123)
    ...
    if message:
      ...
      dialog.addInputField(128, 220, 350, 20, 2100, 2, "Enter message...")
    dialog.addButton(124, 252, 247, 248, 2)
  else:
    dialog.addText(127, 192, "Send this member a private message:", 980)
    dialog.addResizeGump(124, 217, 9350, 355, 27)
    dialog.addText(128, 220, "Enter message...", 980)
  ...
  if char != player and guild.leader != char:
    dialog.addText(127, 298, "Kick this member out of the guild:", 2117)
  ...
  if not guild.leader:
    ...
    dialog.addText(127, 371, "Vote for a new leader:", 980)
...
def guildMembers(player, guild):
  ...
  dialog.addTilePic(106, 49, 3805)
  dialog.addText(160, 90, "Guildstone for %s%s" % (name, abbreviation), 380)
  dialog.addText(160, 113, "Member Overview", 2100)
  dialog.addResizeGump(100, 148, 9200, 405, 355)
  dialog.addGump(14, 468, 10402, 0)
  dialog.addButton(103, 511, 247, 248, 1)
  dialog.addButton(178, 511, 242, 241, 0)
  ...
  for member in members:
    if offset == 0:
      ...
      if page < pages:
        ...
        dialog.addText(401, 468, "Next Page", 980)
    ...
    if len(info['guildtitle']) > 0:
      dialog.addText(259, 223 + offset, "Title: %s" % info['guildtitle'], 2100)
    else:
...
def guildCanidatesOverview(player, guild):
  ...
  if len(offline) > 0:
    html += '<basefont color="#666666"><h3><u>Offline:</u></h3><basefont color="#333333"><br>'
    for name in offline:
      html += '%s<br>' % name
    html += '<br>'

  if len(offline) == 0 and len(online) == 0:
    html += 'There are currently no known canidates for a membership in this guild.'
    html += '<br><br>'
  ...
  for member in guild.members:
    if member == guild.leader:
      recruitment.append(member.name + ' (Leader)')
    elif checkPermission(member, guild, PERMISSION_RECRUIT, 1):
      recruitment.append(member.name)

  if len(recruitment) != 0:
    html += '<basefont color="#666666"><h3><u>Responsible For Recruitment:</u></h3><basefont color="#333333"><br>'
    for member in recruitment:
      html += '%s<br>' % member
    html += '<br><br>'
  else:
    html += 'There is currently no one responsible for recruiting canidates.'
    html += '<br><br>'
  ...
  dialog.addTilePic(106, 49, 3805)
  dialog.addText(160, 90, "Guildstone for %s%s" % (name, abbreviation), 380)
  dialog.addText(160, 113, "Canidate Overview", 2100)
...
def guildCanidates(player, guild):
  if checkPermission(player, guild, PERMISSION_RECRUIT|PERMISSION_EXPEL|PERMISSION_PROMOTE):
    ...
    dialog.addTilePic(106, 49, 3805)
    dialog.addText(160, 90, "Guildstone for %s%s" % (name, abbreviation), 380)
    dialog.addText(160, 113, "Canidate Management", 2100)
    ...
    if checkPermission(player, guild, PERMISSION_RECRUIT):
      ...
      dialog.addText(152, 406, "Recruit a new member for this guild.", 980)
    ...
    for canidate in canidates:
      ...
      if checkPermission(player, guild, PERMISSION_PROMOTE):
        ...
        dialog.addText(338, 189 + offset, "Accept this canidate", 980)
...
def guildProperties(player, guild):
  ...
  if not readonly:
    ...
    dialog.addText(120, 363, "Charta (HTML allowed)", 2100)
    ...
    if guild.alignment == 1:
      ...
      dialog.addText(160, 261 + offset, "Good", 980)
      dialog.addGump(124, 256 + offset, 9721, 980)

    if guild.alignment == 0:
      dialog.addText(286, 261 + offset, "Neutral", 2100)
      dialog.addGump(248, 256 + offset, 9724)
    else:
      dialog.addText(286, 261 + offset, "Neutral", 980)
      dialog.addGump(248, 256 + offset, 9721, 980)

    if guild.alignment == 2:
      dialog.addText(420, 261 + offset, "Evil", 2100)
      dialog.addGump(384, 256 + offset, 9724)
    else:
      dialog.addText(420, 261 + offset, "Evil", 980)
      dialog.addGump(384, 256 + offset, 9721, 980)

    # Add a HTML gump with additional information
    html = ''
    if len(guild.website) != 0:
      html += '<basefont color="#666666"><h3><u>Website:</u></h3><basefont color="#333333"><br><a href="%s">%s</a><br><br>' % (guild.website, guild.website)
    else:
      html += '<basefont color="#666666"><h3><u>Website:</u></h3><basefont color="#333333"><br>This guild has no website.<br><br>'

    if len(guild.charta) != 0:
      html += '<basefont color="#666666"><h3><u>Charta:</u></h3><basefont color="#333333"><br>%s' % (guild.charta)
    else:
      html += '<basefont color="#666666"><h3><u>Charta:</u></h3><basefont color="#333333"><br>This guild has not defined a charta yet.'
  ...
  if response.button == 1 and player in guild.members:
    ...
    if 1 in response.switches:
      if player in guild.members:
        ...
        player.socket.sysmessage('You decide not to show your guildsign any longer.')
        player.resendtooltip()

    elif 2 in response.switches:
      if player in guild.members:
        guild.setmemberinfo(player, {'showsign': 1})
        player.socket.sysmessage('You decide to show your guildsign from now on.')
        player.resendtooltip()
    ...
    # Resign from leadership
      ...
      if player == guild.leader:
        members = guild.members
        guild.leader = None
        player.socket.sysmessage('You have resigned from your position as the leader of this guild.')
        player.resendtooltip()
        for member in members:
          if member != player:
            sendMessage(member, "%s has resigned from the leadership position of your guild. You can elect a new leader by voting for a member at your guildstone." % player.name)
        if guild.guildstone:
          guild.guildstone.resendtooltip()
...
def guildMembership(player, guild):
  ...
  dialog.addTilePic(106, 49, 3805)
  dialog.addText(160, 90, "Guildstone for %s%s" % (name, abbreviation), 380)
  dialog.addText(160, 113, "Membership Options", 2100)
  dialog.addResizeGump(100, 149, 9200, 404, 283)
  dialog.addGump(13, 339, 10402, 0)
  dialog.addButton(102, 450, 247, 248, 1)
  dialog.addButton(177, 450, 242, 241, 0)
  ...
  if player in guild.members or player in guild.canidates:
    dialog.addText(156, 212, "Resign from this guild.", 2100)
    dialog.addRadioButton(116, 208, 9721, 9724, 3, 0)
  else:
    dialog.addText(156, 212, "Resign from this guild.", 980)
...
def mainMenu(player, guild):
  ...
  dialog.addTilePic(106, 49, 3805)
  dialog.addText(160, 90, "Guildstone for %s%s" % (name, abbreviation), 380)
  dialog.addText(160, 113, "Main Menu", 2100)
  dialog.addResizeGump(100, 149, 9200, 404, 283)
  dialog.addGump(13, 339, 10402, 0)
  dialog.addButton(102, 450, 247, 248, 1)
  dialog.addButton(177, 450, 242, 241, 0)
  ...
  if checkPermission(player, guild, PERMISSION_PROPERTIES):
    dialog.addText(156, 172, "Modify the general properties of this guild.", 2100)
    dialog.addRadioButton(116, 168, 9721, 9724, MAINMENU_GENERALPROPERTIES, 0)
  else:
    dialog.addText(156, 172, "View information about this guild.", 2100)
    dialog.addRadioButton(116, 168, 9721, 9724, MAINMENU_GENERALPROPERTIES, 0)

  if checkPermission(player, guild, PERMISSION_KICK):
    dialog.addText(156, 212, "Manage the members of this guild.", 2100)
    dialog.addRadioButton(116, 208, 9721, 9724, MAINMENU_MEMBERS, 0)
  else:
    dialog.addText(156, 212, "View the members of this guild.", 2100)
    dialog.addRadioButton(116, 208, 9721, 9724, MAINMENU_MEMBERS, 0)

  if checkPermission(player, guild, PERMISSION_RECRUIT|PERMISSION_EXPEL|PERMISSION_PROMOTE):
    dialog.addText(156, 252, "Manage the canidates of this guild.", 2100)
    dialog.addRadioButton(116, 248, 9721, 9724, MAINMENU_CANIDATES, 0)
  else:
    dialog.addText(156, 252, "View the canidates of this guild.", 2100)
    dialog.addRadioButton(116, 248, 9721, 9724, MAINMENU_CANIDATES, 0)

  # Diplomatic relations are not done yet
  dialog.addText(156, 292, "View the diplomatic relations of this guild.", 980)
