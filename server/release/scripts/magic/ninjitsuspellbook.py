...
def hasspell( item, spell ):
	if item and item.hasscript( 'magic.ninjitsuspellbook' ):
		spell = spell - 400
...
def addspell( item, spell ):
	if not item or not item.hasscript( 'magic.ninjitsuspellbook' ):
		return 0
...
def onUse(char, item):
	...
	packet.setshort( 13, 501 ) # Scroll offset (1 = regular, 101 = necro, 201 = paladin, 400 = bushido, 500 = ninjitsu, 601 = arcanist)
