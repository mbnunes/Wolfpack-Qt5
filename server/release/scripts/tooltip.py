...
def onShowTooltip( sender, target, tooltip ):  
  if target.isitem():
    ...
    # If the target is a player and in a guild, add that to the tooltip
    if target.player and target.guild:
      info = target.guild.getmemberinfo(target)
      if len(info['guildtitle']) > 0:
        additional.append('%s, %s' % (info['guildtitle'], target.guild.name))
      else:
        additional.append(target.guild.name)
