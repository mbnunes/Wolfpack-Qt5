...
namespace Utilities {
	...
	QString dumpData(const QByteArray &data) {
		...
		QString dumped = QString( "\n[ packet: %1; length: %2 ]\n" ).arg( ( uchar ) data[0], 2, 16 ).arg( data.count() );
		...
		for ( int actLine = 0; actLine < lines; ++actLine )
		{
			...
			for ( ; actRow < 16; ++actRow )
			{
				...
				{
					QString number = QString::number( static_cast<uint>( static_cast<uchar>( data[actLine*16 + actRow] ) ), 16 ) + QString( " " );
			...
			{
				if ( actLine * 16 + actRow < length )
					line += ( isprint( static_cast<uchar>( data[actLine * 16 + actRow] ) ) ) ? data[actLine * 16 + actRow] : '.' ;
			}
