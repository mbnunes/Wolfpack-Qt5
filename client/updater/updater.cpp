...
uint cUpdater::getRemoteVersion() {
	QUrl url("http://www.hartte.de/uoclient/version.php");
	...
	return QString(result).toUInt();
}

bool cUpdater::checkForUpdates() {
	uint remoteBuild = getRemoteVersion();
	uint localBuild = getLocalVersion();

	Log->print(LOG_NOTICE, tr("Local Version: %1; Remote Version: %2.\n").arg(localBuild).arg(remoteBuild));
	...
	if (destination.cd("update")) {
		extractData("update.7z", destination);
	}
	return true;
	...
	cQuestionDialog *dialog = new cQuestionDialog(localBuild, remoteBuild, 0);
	dialog->show();
